...
class TacoTestHelper(Helper):
    def __init__(self, batch_size, hparams, input_seq_lengths):
        with tf.name_scope('TacoTestHelper'):
            self._batch_size = batch_size
            self._reduction_factor = hparams.outputs_per_step
            self.use_all_outputs = hparams.use_all_outputs
            if self.use_all_outputs:
                self._output_dim = hparams.num_mels * self._reduction_factor
            else:
    ...
    def next_inputs(self, time, outputs, state, sample_ids, stop_token_prediction, prev_max_attentions=None, name=None):
        ...
        with tf.name_scope('TacoTestHelper'):
            ...
            #finished = tf.cast(tf.logical_or(tf.cast(tf.round(stop_token_prediction), tf.bool), tf.less_equal(self.input_seq_lengths, prev_max_attentions)), tf.bool)
            #finished = tf.cast(tf.round(stop_token_prediction), tf.bool)
...
class TacoTrainingHelper(Helper):
    def __init__(self, batch_size, targets, hparams, gta, evaluating, global_step):
        ...
        with tf.name_scope('TacoTrainingHelper'):
            ...
            if self.use_all_outputs:
                ...
                self._targets = tf.reshape(targets, (shape[0], -1, self._output_dim))
            else:
                self._output_dim = hparams.num_mels
    ...
    def initialize(self, name=None):
        ...
        if self.gta:
            ...
            self._ratio = tf.convert_to_tensor(2.0) #Force eval model to always feed predictions
        else:
