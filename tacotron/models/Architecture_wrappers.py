...
class TacotronDecoderCellState(
    collections.namedtuple("TacotronDecoderCellState",
     ("cell_state", "attention", "time", "alignments",
      "alignment_history", "max_attentions", "mu", "alpha", "cumulated_alignments", "pos_rec"))):
    ...
    * : This is typically taking a vanilla LSTM, wrapping it using tensorflow's attention wrapper,
    ...
    def state_size(self):
        ...
        return TacotronDecoderCellState(
            ...
            max_attentions=(), 
            mu=1,
            alpha=self._attention_mechanism.alignments_size,
            cumulated_alignments=self._attention_mechanism.alignments_size,
            pos_rec=(),)
    ...
    def zero_state(self, batch_size, dtype):
        ...
        with ops.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
            ...
            return TacotronDecoderCellState(
                ...
                mu=self._attention_mechanism.init_mu,
                alpha=self._attention_mechanism.init_alpha,
                cumulated_alignments=self._attention_mechanism.init_cumulated_alignments,
                pos_rec=tf.zeros((batch_size, ), dtype=tf.int32))
    ...
    def __call__(self, inputs, state):
        ...
        alignments, new_mu, context, cumulated_alignments, max_attentions, pos_rec = self._attention_mechanism(LSTM_output, state)
        #Concat LSTM outputs and context vector to form projections inputs
        projections_input = tf.concat([LSTM_output, context], axis=-1)
