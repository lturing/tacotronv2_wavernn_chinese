...
class TacotronDecoderCell(RNNCell):
    ...
    * : This is typically taking a vanilla LSTM, wrapping it using tensorflow's attention wrapper,
    ...
    def __init__(self, prenet, attention_mechanism, rnn_cell, frame_projection, stop_projection, 
        ...
        self._attention_layer_size = self._attention_mechanism.attention_size 
        self._alignment_size = self._attention_mechanism.alignment_size 
    ...
    def state_size(self):
        ...
        return TacotronDecoderCellState(
            ...
            alignments=self._alignment_size,
            alignment_history=(),
            #alpha=self._num_attn_mixture,
            #beta = self._num_attn_mixture,
            kappa = self._num_attn_mixture,
        )
    ...
    def zero_state(self, batch_size, dtype):
        ...
        with ops.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
            ...
            return TacotronDecoderCellState(
                ...
                attention=tf.zeros((batch_size, self._attention_layer_size), dtype),
                alignments=tf.zeros((batch_size, self._alignment_size), dtype),
                alignment_history=tensor_array_ops.TensorArray(dtype=dtype, size=0,
                dynamic_size=True),
                #alpha=tf.zeros([batch_size, self._num_attn_mixture], dtype=tf.float32),
                #beta = tf.zeros([batch_size, self._num_attn_mixture], dtpe=tf.float32),
                kappa=tf.zeros([batch_size, self._num_attn_mixture], dtype=tf.float32),
                )
    ...
    def __call__(self, inputs, state):
        ...
        alignments, attentions, kappa = self._attention_mechanism(
        LSTM_output, state, self._num_attn_mixture)

        #Concat LSTM outputs and context vector to form projections inputs
        projections_input = tf.concat([LSTM_output, attentions], axis=-1)
